...
<c:choose>
    <c:when test="${param.action == 'insert'}">
        ...
        <sql:update>
            INSERT INTO Student 
            (SID, sName, Password, Email, Birth, Tell, Address)
            VALUES(?,?, ?, ?, ?, ?, ?)
        ...
        <c:redirect url="/Student/CP.jsp?action=status" >                        
            <c:param name="NormalMessage" value="Registration Completed<br />Your ID: <b>${MAXSID.rows[0].SID+1}</b> <br />"/>                        
        </c:redirect>
    ...
    <c:when test="${param.action == 'update'}">
        ...
        <sql:query  var="Student" sql=" SELECT * FROM Student WHERE SID = ${param.SID}" />                
        <c:set var="User" value="${Student.rows[0]}" scope="session" />
