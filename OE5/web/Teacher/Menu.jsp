...
<div id="nav">
    ...
    <li class="${param.action == 'course'? 'current':''}">
        <a href="CP.jsp?action=course">Course Control</a>
        <c:if test="${param.action == 'course'}"><jsp:include page="Course/Menu.jsp" /></c:if>
    </li>    
    
    <li class="${param.action == 'question'? 'current':''}">
        <a href="CP.jsp?action=question">Question Control</a>
        <c:if test="${param.action == 'question'}"><jsp:include page="Question/Menu.jsp" /></c:if>
    </li> 
    ...
    <li class="${param.action == 'group'? 'current':''}">
        <a href="CP.jsp?action=group&subaction=list&FTID=0">Groups Control</a>
        <c:if test="${param.action == 'group'}"><jsp:include page="Group/Menu.jsp" /></c:if>
    </li>  
  
    <li class="${param.action == 'student'? 'current':''}">
        <a href="CP.jsp?action=student">Student Control</a>
        <c:if test="${param.action == 'student'}"><jsp:include page="Student/Menu.jsp" /></c:if>
    </li>
</div>